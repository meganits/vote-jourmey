...
int numCandidates = 0;
VoterInfo voters[MAX_VOTERS];
int numVoters = 0;

// Function to create election schedule
...
// Function to search voter details
void searchVoterDetails() {
    int voterSno;
...
// Function to display candidates for a constituency
void displayCandidates(const char *constituency) {
...
void adminLogin() {
	char UserName[20];
			...
			printf("Password:");
			scanf("%s",password);
			if(strcmp(UserName,"APU")==0 && strcmp(password,"@pu123")==0)
					printf("Welcome Admin, ACCESS GRANTED \n");
    return;
}
